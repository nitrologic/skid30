...
;
;v190 - attempting to add [] checking...
;
;v200 - run prog does a 'createproc' so user can do a PANIC!
;       Blitz2 loads debugger now instead of dhandlerlib
;
;v190 - added asmexit...checked for func ret outside of func.
;
;**************************************************;
...
tok35	dc.l	tok36
	dc	35
	dc.b	'Jimi',0,0	;'CAnd',0,0	;available
	even
tok36	dc.l	tok37
	dc	36
	dc.b	'Jimi',0,0	;'COr',0,0	;available
	even
	;
...
verreqla5	dc.b	'(C)opyright 1993 - Acid Software',0
	even
verreqin3	dc.b	1,0,1,0
	...
	dc	64-8-16,30
	dc.l	0,verreqla6,0
verreqla6	dc.b	'Amiga Format Demo Version',0
	endc

	ifne	demo
	dc	64-8,30
	dc.l	0,verreqla6,0
verreqla6	dc.b	'Demonstartion Version',0
	endc

	ifne	release
	dc	64+16,30
	dc.l	0,verreqla6,0
verreqla6	dc.b	'Release Version',0
	endc
...
atokens	dc.l	donewtype,doend,dolet,dodim,dogoto
	...
	dc.l	dodefault,eos
	dc.l	doxinclude,doinclude,dounless
	...
	dc.l	docerr,dothen,doelse,syntaxerr,syntaxerr
	;
...
debugname	dc.b	'blitz2:dbug/defaultdbug',0
	even
dseg	dc.l	0
...
deflibs	dc	0	;flag for if deflibs are used
deflibslen	dc.l	0	;length of def libs
...
makelibs	;find all lib dirs in 'libsdir' and make a list of 'em
	...
	move.l	#namebuff,d2
	jsr	examine(a6)
	move.l	namebuff+124,deflibslen
	move.l	d7,d1
	jmp	unlock(a6)
	;
lnodefs	move.l	#libsdir,d1
	moveq	#-2,d2
	jsr	lock(a6)
	move.l	d0,d7
	beq	.done
...
.okloop	clr	(a0)+
	dbf	d0,.okloop
	;
	;put in 3 nops for dummy JSR to init routines!
	;
	lea	nops3,a0
	lea	nopsf,a1
	bsr	pokecode2
...
.nono	;check if asm type things....
	;
	;if so, no line remembering etc!, optimizing
	;
	bclr	#15,d0
	move	d0,d1
	and	#$7f00,d1
...
loadlibs	;use libslist to load in used libs dir contents
	...
	beq	.nodefs
	;
	move.l	deflibslen,d0
	moveq	#1,d1
	move.l	4.w,a6
	jsr	doallocmem	;get memory for 'deflibs'
	move.l	d0,deflibsat
	;
	move.l	dos,a6
	move.l	#defname,d1
	...
	beq	.nodefs
	move.l	d7,d1
	move.l	deflibsat(pc),d2
	move.l	deflibslen,d3
	jsr	read(a6)
	move.l	d7,d1
	jsr	close(a6)
	;
...
loadalib	;filename pointed to by d1	;dos base in a6
	...
	move.l	dos,a6
	;
	move.l	d0,a5		;loc of struct in mem
	move.l	a5,(a3)
	move.l	a5,a3
	;
	move.l	d7,d1
...
;	move	#65235,d1
;	bsr	findlib
;	move.l	a1,stringslib1	;and string lib1
;	move	#64535,d1
;	bsr	findlib
;	move.l	a1,datalib
.cont	rts
...
;	bsr	dopusha3
;.sss	;
;	cmp	#7,2(a7)	;is it a string type thing?
;	bne	.sssz
;	move.l	a2,-(a7)
;	bsr	makesbase	;we'll need string base then.
;	move.l	(a7)+,a2
