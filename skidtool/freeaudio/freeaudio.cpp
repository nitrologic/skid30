...
sound *mixer::play(sample *sam,output *out,int state,int nowms)
{
	sound	*s;
...
void mixer::mix(int count,int nowms)
{
	...
	while (sptr=startlist.pull())
	{
		sptr->next=playlist;
		playlist=sptr;
		int skip=20+sptr->starttime-nowms;
		if (skip<0) skip=0;
		skip=(skip*freq)/1000;
		sptr->delay=skip;		
	}
...
void mixer::mix8(u8 *abuff,int count,int nowms)
{
	int 	i,j;
	int 	*tptr;
...
void mixer::mix16(short *d,int count,int nowms)
{
	int *tptr;
...
void mixer::mix16s(short *dd,int count,int nowms)
{
	int	*d,*tptr;
	if (count==0) count=size;
	mix(count,nowms);
