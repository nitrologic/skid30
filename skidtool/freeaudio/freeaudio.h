...
struct mixer:output
{
	...
	sound *play(sample *s,output *out,int state,int nowms);

	void releasesound(sound *s);
	void releaseall();

	void mix8(u8 *abuff,int count,int nowms);
	void mix16(short *abuff,int count, int nowms);
	void mix16s(short *abuff,int count, int nowms);
	void mix(int count,int nowms);
};
...
// virtual OS dependent interface
	...
	sound *play(sample *sam,output *out,int paused,int nowms)
	{
		sound	*snd;
		int		freq,vol,state;
		...
		snd=mix->play(sam,out,state,nowms);
		return snd;
	}
};
